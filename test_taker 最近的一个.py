...
def inference(cwd,two_way_propagation,  middle_pic_nums):
    ...
    checkpoint_path=os.path.join(cwd, "UNet_model")
    ...
    [os.makedirs(os.path.join(output_test_crop_dir, dir_name), exist_ok=True) for dir_name in ['', 'images', 'labels']]
    ...
    for video in sorted(os.listdir(os.path.join(output_test_dir, 'images'))):
        [os.makedirs(os.path.join(output_test_crop_dir, i, video), exist_ok=True) for i in ['images','labels']]
        print('you are processing video ', video)
        # video的名字是video1,video2,video3...
        # ///////////////////////////////////////////////////
        # video的名字是video1,video2,video3...
        for item in sorted(os.listdir(os.path.join(output_test_dir, 'labels', video))):
            # 新建一个label文件夹用于反复存储测试结果而不改变原文件夹
            src = os.path.join(output_test_dir, 'labels', video, item)
            dst = os.path.join(result_output_dir,'labels', video, item)
        ...
        for item in sorted(os.listdir(os.path.join(result_output_dir, 'labels', video))):
            tailor(256, 256, os.path.join(result_output_dir, 'labels', video, item), os.path.join(output_test_crop_dir, 'labels', video), region=32)
        ...
        for img in imgList:
            if img.endswith(".png"):
                ...
                if int(name) > min_file and int(name) <= max_file:
                    ...
                    stitch(256, 256, name, os.path.join(output_test_crop_dir,'labels', video), os.path.join(result_output_dir, 'labels', video, name + ".png"), 32)
                    n += 1
