...
from WPU_Net_master.segmentation.data_U_DPU import IronDataset
...
class Trainer(object):
    ...
    def __init__(self, bs=24, loss_type='abw', multi_layer=True, data_dir = '',workers=1):
        ...
        train_vid=(IronDataset(train_vid_list,train_label_list, train=True, transform=self.transform, crop=True, crop_size=(256, 256), dilate=3))
        # check(self.train_dataset)
        self.train_loader = DataLoader(train_vid, batch_size=self.batch_size, shuffle=True, num_workers=workers)
        ...
        val_vid=(IronDataset(val_vid_list,val_label_list, train=False, transform=self.transform, crop=False, crop_size=(256, 256), dilate=3))
        #check(self.test_dataset)
        self.test_loader = DataLoader(val_vid, batch_size=self.batch_size, shuffle=False, num_workers=workers)
    ...
    def train(self, epoch, loss_list, plotimgs,temporary_folder):
        ...
        for batch_idx, (image, mask, last, next, weight) in enumerate(F):
            images = []
            label = mask.squeeze(1).long()

            if torch.cuda.is_available():
                img, label, last, next, weight = image.cuda(), label.cuda(), last.cuda(),next.cuda(), weight.cuda()
            ...
            _,downsample_result = self.model(img, last, next)
            torch.save(downsample_result,os.path.join(downsample_folder,"batch_no."+str(batch_idx)+"_of this epoch.pt"))
        
            downsample_result=torch.load(os.path.join(downsample_folder,"batch_no."+str(batch_idx)+"_of this epoch.pt"))
            output= self.model(img, next, downsample_result)
    ...
    def test(self, epoch, test_loss_list,temporary_folder):
        ...
        for batch_idx, (image, mask, last, next, weight) in enumerate(F):
            ...
            if torch.cuda.is_available():
                label = mask.squeeze(1).long()  
                img, label, last, next, weight = image.cuda(), label.cuda(), last.cuda(), next(), weight.cuda()
            ...
            next[next == 255] = -6
            next[next == 0] = 1
            downsample_folder=os.path.join(cwd,"temporary_data","val","epoch_"+str(epoch))
            os.makedirs(downsample_folder,exist_ok=True)

            _,downsample_result = self.model(img, last)
            torch.save(downsample_result,os.path.join(downsample_folder,"batch_no."+str(batch_idx)+"_of this epoch.pt"))
...
if __name__ == "__main__":
    ...
    checkpoint_path = os.path.join(cwd, "DPU_Net_model/",current_time)
