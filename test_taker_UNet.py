...
import shutil
def inference(cwd,two_way_propagation,  middle_pic_nums):
    '''
    ...
    model_folder=os.path.join(cwd, "UNet_model")
    # Find the most recent model folder
    model_folders = glob.glob(os.path.join(model_folder, "*"))
    most_recent_model_folder = max(model_folders, key=os.path.getctime)
    # Use the most recent model folder as the checkpoint path
    checkpoint_path = os.path.join(most_recent_model_folder)

    model_path = os.path.join(checkpoint_path, '_best_model_state.pth')
    ...
    [os.makedirs(os.path.join(output_test_crop_dir, dir_name), exist_ok=True) for dir_name in ['', 'images', 'Annotations']]
    result_output_dir=os.path.join(cwd, "datasets", "segmentation", "results_UNet", "label_results")
    ...
    # result_save_dir = os.path.join(cwd, "datasets", "segmentation", "net_test", "test_overlap_crop",'Annotations')
    ...
    model.eval()
    for video in sorted(os.listdir(os.path.join(output_test_dir, 'images'))):
        [os.makedirs(os.path.join(output_test_crop_dir, i, video), exist_ok=True) for i in ['images','Annotations']]
        ...
        # 完成复制。
        # ////////////////////////////////////////////////////
        # print(sorted(os.listdir(os.path.join(result_output_dir, 'Annotations', video))))
        for item in sorted(os.listdir(os.path.join(output_test_dir, 'images', video))):
            tailor(256, 256, os.path.join(output_test_dir, 'images', video, item), os.path.join(output_test_crop_dir, 'images', video), region=32)
        for item in sorted(os.listdir(os.path.join(result_output_dir, 'Annotations', video))):
            tailor(256, 256, os.path.join(result_output_dir, 'Annotations', video, item), os.path.join(output_test_crop_dir, 'Annotations', video), region=32)

        # inferece cropped images
        images = sorted(os.listdir(os.path.join(output_test_crop_dir, "images", video)))
        ori_images = sorted(os.listdir(os.path.join(output_test_dir, "images", video)))
        ...
        downsample_folder=os.path.join(cwd,"temporary_data","test",str(video))
        ...
        imgList = sorted(os.listdir(os.path.join(output_test_dir, "images", video)))
        print(len(imgList))
        n = 0
        for img in imgList:
            if img.endswith(".png"):
