...
def tailor(sizeX, sizeY, oriAddress, saveAddress, region = 32):
    ...
    #img_name = os.path.splitext(os.path.basename(oriAddress))[0]
    print("you are cropping picture : ", oriAddress, "img name : ", img_name)
...
def rand_crop(data, label, height, width, last=None, next=None):
    """
    ...
    #这里的data.size[1]是宽度，data.size[0]是高度
    ...
    if last is not None and next is not None:
        ...
        last = last.crop(box)
        next = next.crop(box)
        return data, label, last, next

    return data, label


def rand_rotation(data, label, last=None, next=None):
    """
    ...
    if last is not None and next is not None:
        ...
        next = F.rotate(next, angle, expand=True)
        return data, label, last, next

    return data, label


def rand_verticalFlip(data, label, last=None, next=None):
    """
...
def rand_horizontalFlip(data, label, last=None, next=None):
    """
