...
class IronDataset(Dataset):
    ...
    def __init__(self,video_list,label_list, train=True, transform=None, crop = True, crop_size=(400, 400), dilate = 5, forward_or_backward_last_count=1):
        """
            ...
            if file.endswith(".png") or file.endswith(".tif") or file.endswith(".jpg"):
                ...
                if self.count==1:
                    if pic_num != min_file:
                        ...
                        if self.train:
                            file_last = str(int(filename) - self.count).zfill(3)
                            # # 双向传播，要准备最后一张图片
                            # file_opposite_last= str(int(filename) + 1).zfill(3)
                        else:
                            file_last =str(int(pic_num) - self.count).zfill(3) +  '_' + filename.split('_')[1] + '_' + filename.split('_')[2] 
