...
def eval_RI_VI(results_dir, outAd, gt_dir):
    """
...
def eval_F_mapKaggle(results_dir, outAd,  gt_dir):
    print("eval_F_mapKaggle")
    ...
    for r in results_path:
        ...
        if os.path.exists(gt_path):
            ...
            print("Before calculating F_test")
            F_test = get_figure_of_merit(result, gt)
            print("After calculating F_test")

            F += F_test     

            #这里卡住了！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
            # print("Before calculating mAP_test")
            # mAP_test = get_map_2018kdasb_new(result, gt)
            # print("After calculating mAP_test")
            # mAP += mAP_test

            out = open(outAd, "a")  # # record the result
            #line = str(F_test) + "," + str(F / count) + "," + str(mAP_test) + "," + str(mAP / count) + "\n"
            line = str(F_test) + "," + str(F / count) + "," + str(mAP / count) + "\n"
            out.write(line)
            out.close()
...
#     os.makedirs(RI_save_dir,exist_ok=True)

#     Map_save_dir = "./evaluation/big_F_mAP/"
#     os.makedirs(Map_save_dir,exist_ok=True)

#     print("WPU_Net_model " + "#####"*20)
#     eval_RI_VI(os.path.join(cwd,"/WPU_Net_model/"), RI_save_dir+"WPU_Net_model.txt")
#     eval_F_mapKaggle(os.path.join(cwd,"/WPU_Net_model/"), Map_save_dir+"WPU_Net_model.txt")
