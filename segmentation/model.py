...
class WPU_Net(nn.Module):
    def __init__(self, num_channels=1, num_classes=2, multi_layer=True):
        ...
        self.fusion_layer = nn.Conv2d(num_feat[5], num_feat[5], kernel_size=1)
        self.upconv1 = Conv3x3(num_feat[4], num_feat[3])
    ...
    def forward(self, inputs, last, opposite_result=None):
        ...
        if opposite_result==None:
            ...
            #特征融合网络（Feature Fusion Network）
            # bottom_feat=(self.fusion_layer(torch.cat([self.bottom(down4_feat), opposite_result], 1)))
            # swiftup=self.larger_scale_up1
            #特征相乘
            result=self.bottom(down4_feat)
            bottom_feat=result * opposite_result
            bottom_feat[bottom_feat < 10] = 0
            swiftup=self.up1
            # bottom_feat = torch.cat([self.bottom(down4_feat), opposite_result], 1)
